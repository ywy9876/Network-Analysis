...
void read_graph(const string file_name) {
	try {
		...
		if (myfile.is_open()) {
			...
			// call to johnson_allpairs
			  Edge edge_array[E.size()] = {};
			  for (int jj=0; jj<E.size(); ++jj){
				  std::string s1 = E[jj].first;
				  std::string s2 = E[jj].second;

				  int i1 = nodeIndex[s1];
				  int i2 = nodeIndex[s2];
				  edge_array[jj] = Edge(i1,i2);
			  }

//			  for (int ii=0; ii<(sizeof(edge_array)/sizeof(*edge_array)); ++ii){
//				  cout << edge_array[ii].first << " " << edge_array[ii].second << endl;
//			  }
...
//
//	 Edge edge_array[Esize] = { };
//	 for (int ii=0; ii<Esize; ii++){
//		 int i1 = mg.nodeIndex[mg.E[ii].first];
//		 int i2 = mg.nodeIndex[mg.E[ii].second];
//		 edge_array[ii]= Edge(i1,i2);
//	 }
//	  int weights[mg.E.size()] = {1};
...
//	 const int V = n;
//	 typedef std::pair < int, int >Edge;
//	 int Esize = edge_vect.size();
//	 Edge edge_array[Esize] = {};
//	 int jj=0;
//	 for (std::vector<std::pair<std::string, std::string>>::iterator it=edge_vect.begin(); it!=edge_vect.end(); ++it){
//			edge_array[jj] = Edge(indexNode[it->first],indexNode[it->second]);
//			jj++;
//
//	 }
//
